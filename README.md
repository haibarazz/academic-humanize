...
项目覆盖完整流程：从学术语料构造 AI-like draft，到 QLoRA SFT，再到 SPIN-style DPO 和迭代 DPO，最后用自动语义指标与 LLM-as-Judge 同时评估语义保真和自然度。
...
- 我设计了 Academic Humanize V2 数据格式：`instruction + AI-like input -> human reference output`。
...
训练样本采用“AI-like draft -> human reference”的结构。每条样本都包含：
...
这样模型可以学习如何从模板化、过度润色、AI-like 的表达回到更自然的学术英文。
