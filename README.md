...
- **本地 7B 后训练闭环**：基于 Qwen2.5-7B-Instruct 构建 QLoRA SFT、SPIN-style DPO 和迭代 DPO，而不是只依赖 API prompt。
...
- **评测不是单一分数**：同时使用 BERTScore-F1、chrF++、BLEU、TER、格式诊断和六维 LLM-as-Judge。
- **有可比较 baseline**：对比 SFT、DPO-v1、DPO-v2 和多个 API 模型，验证本地 adapter 的优势和边界。

## 作者主要完成

- 设计 Academic Humanize V2 数据格式：`instruction + AI-like input -> human reference output`。
- 实现 QLoRA SFT 训练、LoRA 推理、API baseline 推理、断点续跑和并发调用。
- 设计 SPIN-style DPO pair 构造流程，并完成 DPO-v1 与迭代 DPO-v2 训练闭环。
- 建立自动 metrics 与 LLM-as-Judge 双层评测框架，固定 judge prompt 和六维评分标准。
- 整理开源版本，包括 toy examples、配置文件、训练脚本、评测脚本和 README 可视化说明。
...
这个例子体现了项目的核心目标：去掉 `not only...but also...`、`facilitates`、`leverages`、`enhance` 这类常见 AI 写作痕迹，同时保留 `heterogeneous signals`、`robust optimization` 和 `12 experimental settings` 等关键信息。
...
Prompt 在这个项目里不是临时字符串，而是可版本管理的实验资产：
...
- LLM-as-Judge 不是人工评审的替代品，因此本项目同时保留自动语义指标做交叉验证。
- 当前验证集规模为 346 条 Academic Humanize 样本，迁移到其他学科或写作风格时需要重新评估。
- DPO 会提升偏好分数，但可能带来语义漂移；DPO-v2 的目标是缓解这个问题，而不是完全消除。
